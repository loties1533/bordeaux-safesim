...
window.updateMapPoints = function(points) {
    points.forEach(p => {
        ...
        const floodLine = p.flood_req !== null
            ? `<div>INONDABLE À: <b style="color: #b45309;">&ge; ${p.flood_req}m NGF</b></div>`
            : `<div style="color: #64748b;">INACCESSIBLE AU FLEUVE</div>`;

        const ppriLine = p.zone_ppri !== null
            ? `ZONE PPRI: <b>${p.zone_ppri}</b>`
            : `<span style="color:#64748b">Hors zone PPRI</span>`;
        ...
        const popupContent = `
            <div style="font-family: 'Share Tech Mono', monospace; min-width: 180px; color: #1e293b;">
                ...
                <div style="margin-top: 5px; font-size: 12px; line-height: 1.4;">
                    ...
                    <div style="background: #fefce8; padding: 2px 4px; margin-top: 2px; font-size: 11px;">
                        ${floodLine}
                    </div>
                    <div style="font-size: 11px; margin-top: 2px; color: #475569;">${ppriLine}</div>
                </div>
